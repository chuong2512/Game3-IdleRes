...
public class SellManagerPopup : MonoBehaviour
{
    ...
    [SerializeField] private Text effective;
    ...
    public void Show(ManagerProfile profile, Action sellHalf, Action sellFull)
    {
        ...
        this.experience.gameObject.GetComponent<Outline>().effectColor =
            Singleton<GameProcess>.Instance.GetManagerBorderColor(profile.experience);
        GameUtilities.String.ToText(this.duration,
            "Duration: " +
            GameUtilities.DateTime.Convert(
                Singleton<GameProcess>.Instance.GetManagerSkillDuration(profile.experience, profile.skill)));
        switch (profile.skill)
        {
            case ManagerSkill.UpgradeCost:
                GameUtilities.String.ToText(this.effective,
                    "-" + Singleton<GameProcess>.Instance.GetManagerSkillEffective(profile.experience, profile.skill) +
                    " Upgrade Cost");
                break;
            case ManagerSkill.WalkingSpeed:
                GameUtilities.String.ToText(this.effective,
                    "x" + Singleton<GameProcess>.Instance.GetManagerSkillEffective(profile.experience, profile.skill) +
                    " Walking Speed");
                break;
