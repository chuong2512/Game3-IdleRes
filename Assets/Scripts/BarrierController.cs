...
public class BarrierController : MonoBehaviour
{
        ...
        public bool MoveNext()
        {
            ...
            switch (num)
            {
                case 0u:
                    ...
                    this._totalDuration___0 = this._this.config
                        .barrier[this._kitchenCount___0 / this._this.config.kitchen.barrierStep - 1].unlockDuration;
                    break;
                case 1u:
                    break;
                default:
            ...
            if (this._this.barrierData.unlockRemaining > 0)
            {
                ...
                GameUtilities.String.ToText(this._this.process.remainingText,
                    GameUtilities.DateTime.Convert(this._this.barrierData.unlockRemaining));
                GameUtilities.String.ToText(this._this.process.diamondText,
                    ((int)Math.Ceiling((double)((float)this._this.config
                                                    .barrier[
                                                        this._kitchenCount___0 / this._this.config.kitchen.barrierStep -
                                                        1].diamondToUnlock *
                                                this._percent___1))).ToString());
    ...
    [SerializeField] private BarrierController.General general;

    [SerializeField] private BarrierController.Enable enable;

    [SerializeField] private BarrierController.Locked locked;

    [SerializeField] private BarrierController.Process process;
    ...
    public void Initialize()
    {
        ...
        expr_15.onCashChange =
            (Action<double>)Delegate.Combine(expr_15.onCashChange, new Action<double>(this.OnCashChange));
        GameManager expr_3B = Singleton<GameManager>.Instance;
        expr_3B.onIdleCashChange =
            (Action<double>)Delegate.Combine(expr_3B.onIdleCashChange, new Action<double>(this.OnIdleCashChange));
    ...
    public void Refresh()
    {
        ...
        if (flag && flag2 && flag3)
        {
            ...
            this.enable.unlockCashButton.sprite = ((Singleton<GameManager>.Instance.database.cash >= floorPrice)
                ? this.general.buttonEnableSprite
                : this.general.buttonDisableSprite);
        ...
        else if (flag && !this.general.barrierEnabled.activeInHierarchy)
        {
            ...
            if (state != BarrierState.Locked)
            {
                ...
                else
                {
                    ...
                    GameUtilities.String.ToText(this.process.reduceTimeText,
                        "-" + GameUtilities.DateTime.Convert(this.config
                            .barrier[count / this.config.kitchen.barrierStep - 1].reduceProcessTime));
            ...
            else
            {
                ...
                double num2 = Singleton<GameProcess>.Instance.GetFloorPrice(count) /
                              (double)this.config.kitchen.unlockBarrierFactor;
                GameUtilities.String.ToText(this.locked.barrierPriceText, GameUtilities.Currencies.Convert(num2));
                GameUtilities.String.ToText(this.locked.processTimeText,
                    GameUtilities.DateTime.Convert(this.config.barrier[count / this.config.kitchen.barrierStep - 1]
                        .unlockDuration));
                this.locked.unlockButtonImage.sprite = ((Singleton<GameManager>.Instance.database.cash >= num2)
                    ? this.general.buttonEnableSprite
                    : this.general.buttonDisableSprite);
            }
        }
    ...
    {
        int count = Singleton<GameManager>.Instance.kitchenController.Count;
        double num = Singleton<GameProcess>.Instance.GetFloorPrice(count) /
                     (double)this.config.kitchen.unlockBarrierFactor;
        ...
        this.barrierData.unlockRemaining =
            this.config.barrier[count / this.config.kitchen.barrierStep - 1].unlockDuration;
    ...
    {
        int count = Singleton<GameManager>.Instance.kitchenController.Count;
        float num = (float)this.barrierData.unlockRemaining /
                    (float)this.config.barrier[count / this.config.kitchen.barrierStep - 1].unlockDuration;
        int diamond =
            (int)Math.Ceiling((double)((float)this.config.barrier[count / this.config.kitchen.barrierStep - 1]
                .diamondToUnlock * num));
        if (Singleton<GameManager>.Instance.database.diamond < diamond)
        {
            Notification.instance.Warning("Not Enough Diamond");
    ...
    private void OnCashChange(double cash)
    {
        ...
        {
            double floorPrice = Singleton<GameProcess>.Instance.GetFloorPrice(count);
            this.enable.unlockCashButton.sprite = ((cash >= floorPrice)
                ? this.general.buttonEnableSprite
                : this.general.buttonDisableSprite);
        }
        else if (this.general.barrierDisable.activeInHierarchy)
        {
            ...
            double num = Singleton<GameProcess>.Instance.GetFloorPrice(count) /
                         (double)this.config.kitchen.unlockBarrierFactor;
            this.locked.unlockButtonImage.sprite =
                ((cash >= num) ? this.general.buttonEnableSprite : this.general.buttonDisableSprite);
        }
    }
    ...
    {
        BarrierController._Processing_c__Iterator0 _Processing_c__Iterator =
            new BarrierController._Processing_c__Iterator0();
