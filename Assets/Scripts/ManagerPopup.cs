...
public class ManagerPopup : Singleton<ManagerPopup>
{
    private sealed class _SellConfirm_c__AnonStorey0
    {
        ...
        internal void __m__0()
        {
            ...
            Notification.instance.Warning("Manager sold for <color=#FBFF00FF>" + GameUtilities.Currencies.Convert(num) +
                                          "</color> coin");
        }
    ...
    [SerializeField] private Text managerPriceText;

    [SerializeField] private Image hireManagerButton;

    [SerializeField] private GameObject targetPopup;

    [SerializeField] private GameObject managerItem;

    [SerializeField] private GameObject hireSpecialButton;

    [SerializeField] private RectTransform assignContent;

    [SerializeField] private RectTransform unassignContent;

    [SerializeField] private SellManagerPopup sellManager;

    [SerializeField] private GameObject tutorial_6;
    ...
    public void Show(KitchenController kitchen)
    {
        ...
        double managerPrice = Singleton<GameProcess>.Instance.GetManagerPrice(
            Singleton<DataManager>.Instance.database.restaurant[this.targetRestaurant].kitchenRecruitment,
            Location.Kitchen);
        this.hireManagerButton.sprite = ((Singleton<GameManager>.Instance.database.cash < managerPrice)
            ? this.disableSprite
            : this.enableSprite);
    ...
    public void Show(ElevatorController elevator)
    {
        ...
        double managerPrice = Singleton<GameProcess>.Instance.GetManagerPrice(
            Singleton<DataManager>.Instance.database.restaurant[this.targetRestaurant].elevatorRecruitment,
            Location.Elevator);
        this.hireManagerButton.sprite = ((Singleton<GameManager>.Instance.database.cash < managerPrice)
            ? this.disableSprite
            : this.enableSprite);
    ...
    public void Show(RestaurantController restaurant)
    {
        ...
        double managerPrice = Singleton<GameProcess>.Instance.GetManagerPrice(
            Singleton<DataManager>.Instance.database.restaurant[this.targetRestaurant].restaurantRecruitment,
            Location.Restaurant);
        this.hireManagerButton.sprite = ((Singleton<GameManager>.Instance.database.cash < managerPrice)
            ? this.disableSprite
            : this.enableSprite);
    ...
    private void OnCashChange(double cash)
    {
        ...
        if (this.kitchen != null)
        {
            num = Singleton<GameProcess>.Instance.GetManagerPrice(
                Singleton<DataManager>.Instance.database.restaurant[this.targetRestaurant].kitchenRecruitment,
                Location.Kitchen);
        }
    ...
    private void HireManagerApply(bool noJunior)
    {
        ...
        if (this.elevator != null)
        {
            ...
            ManagerProfile managerProfile =
                Singleton<GameProcess>.Instance.GetManagerProfile(Location.Elevator, num, noJunior);
        ...
        if (this.restaurant != null)
        {
            ...
            ManagerProfile managerProfile2 =
                Singleton<GameProcess>.Instance.GetManagerProfile(Location.Restaurant, num, noJunior);
        ...
        if (this.kitchen != null)
        {
            ...
            ManagerProfile managerProfile3 =
                Singleton<GameProcess>.Instance.GetManagerProfile(Location.Kitchen, num, noJunior);
