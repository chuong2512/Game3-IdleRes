...
public class BoostManager : MonoBehaviour
{
    ...
    private sealed class _Boosting_c__Iterator0 : IEnumerator, IDisposable, IEnumerator<object>
    {
        ...
        public bool MoveNext()
        {
            ...
            if (this._this.totalRemaining > 0)
            {
                ...
                GameUtilities.String.ToText(this._this.inventoryRemaining,
                    GameUtilities.DateTime.Convert(this._this.totalRemaining));
                GameUtilities.String.ToText(this._this.mainScreenRemaining,
                    GameUtilities.DateTime.Convert(this._this.totalRemaining));
    ...
    [NonSerialized] public int totalEffective;

    [NonSerialized] public BoostData boostData;

    [SerializeField] public Configuration configuration;

    [SerializeField] private Image boostBorder;

    [SerializeField] private Image nextBoostFill;

    [SerializeField] private Image currentBoostFill;

    [SerializeField] private Image[] boostItem;

    [SerializeField] private BoostManager.SpriteBoost[] spriteBoost;

    [SerializeField] private Text boostDescription;

    [SerializeField] private Text inventoryEffective;

    [SerializeField] private Text inventoryRemaining;

    [SerializeField] private Text mainScreenEffective;

    [SerializeField] private Text mainScreenRemaining;
    ...
    public void TotalEffectiveCompute()
    {
        ...
        for (i = 0; i < this.boostItem.Length; i++)
        {
            ...
            {
                this.boostItem[i].transform.GetChild(1).GetComponent<Image>().sprite = Array
                    .Find<BoostManager.SpriteBoost>(this.spriteBoost,
                        (BoostManager.SpriteBoost target) => target.effective == this.boostData.boosts[i].effective)
                    .sprite;
                this.boostItem[i].transform.GetChild(1).GetComponent<Image>().SetNativeSize();
            }
        }

        int num2 = (!Singleton<DataManager>.Instance.database.nonConsume.Contains("onlinepack"))
            ? 1
            : this.configuration.boost.onlineBoostEffective;
        ...
        this.enablePanel.SetActive(this.totalRemaining > 0 ||
                                   Singleton<DataManager>.Instance.database.nonConsume.Contains("onlinepack"));
        this.disablePanel.SetActive(this.totalRemaining == 0 &&
                                    !Singleton<DataManager>.Instance.database.nonConsume.Contains("onlinepack"));
    }
    ...
    {
        int targetRestaurant = Singleton<DataManager>.Instance.database.targetRestaurant;
        int num = GameUtilities.DateTime.Offline(Singleton<DataManager>.Instance.database.restaurant[targetRestaurant]
            .dateTime);
