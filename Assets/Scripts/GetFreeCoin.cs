...
public class GetFreeCoin : MonoBehaviour
{
            ...
            switch (num)
            {
                ...
                case 1u:
                    if (this._www___0.error == null)
                    {
                        ...
                        this._this.currentTime = this._this.currentTime.AddSeconds(Convert.ToDouble(this._www___0.text))
                            .ToLocalTime();
                        DateTime dateTime = Convert.ToDateTime(this._this.freeCoinData.lastTimeGetFree);
                        if ((dateTime.Day != this._this.currentTime.Day ||
                             dateTime.Month != this._this.currentTime.Month) &&
                        ...
                        {
                            int num2 = (int)this._this.currentTime
                                .Subtract(Convert.ToDateTime(this._this.freeCoinData.lastTimeWatchAd)).TotalSeconds;
        ...
        public bool MoveNext()
        {
            ...
            switch (num)
            {
                case 0u:
                    ...
                    this._duration___0 = (int)DateTime.Now
                        .Subtract(Convert.ToDateTime(this._this.freeCoinData.lastTimeWatchAd)).TotalSeconds;
                    this._duration___0 = Mathf.Clamp(this._this.config.freeCash.cooldownPerAds - this._duration___0, 0,
            ...
            {
                GameUtilities.String.ToText(this._this.watchAdsRemaining,
                    GameUtilities.DateTime.Convert(this._duration___0));
    ...
    public void GetFreeCash()
    {
        ...
        Notification.instance.Warning("Received <color=#00FFDFFF>" + this.config.freeCash.diamondBonus.ToString() +
                                      "</color> diamond");
